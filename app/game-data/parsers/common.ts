...
import type {
  VendorColor,
  VendorItemAmount,
  VendorSizeInFoundations3D,
} from "~/game-data/parsers/types";
...
export function parseFullClassNames(
  quotedFullClassNames: string[],
): Effect.Effect<string[], ParseError> {
  return pipe(
    quotedFullClassNames,
    Array.map(parseFullClassName),
    Effect.allSuccesses,
  );
}

export function parseFullClassName(
  quotedFullClassName: string,
): Effect.Effect<string, ParseError> {
  return pipe(
    ...
    Effect.andThen((fullClassName) => {
      ...
      if (lastDotIndex === -1) {
        return Effect.fail(
          new ParseError(
            `Full Class name ("${fullClassName}") didn't match expected format.`,
          ),
        );
      }
...
export function parseItemAmounts(
  data: string,
): Effect.Effect<VendorItemAmount[], ParseError> {
  return pipe(
    parseVendorList(data),
    Effect.andThen(Effect.forEach(parseItemAmount)),
  );
}
...
export function parseItemAmount(
  ...
  return parseVendorMap(data).pipe(
    Effect.flatMap((values) => {
      ...
      return Effect.all([itemClassEffect, amountEffect]).pipe(
        Effect.andThen(
...
export function parseColor(
  value: string,
): Effect.Effect<VendorColor, ParseError> {
  return pipe(
    parseVendorMap(value),
    Effect.flatMap((rawEntries) =>
      pipe(
        rawEntries,
        Array.map(([rawKey, rawValue]) =>
          pipe(Effect.all([rawKeyToKey(rawKey), parseVendorInt(rawValue)])),
        ),
        Effect.allSuccesses,
        Effect.map(
          (entries) =>
...
export function parseVendorSizeInFoundations3D(
  value: string,
): Effect.Effect<VendorSizeInFoundations3D, ParseError> {
  return pipe(
    parseVendorMap(value),
    Effect.flatMap((rawEntries) =>
      pipe(
        rawEntries,
        Array.map(([rawKey, rawValue]) =>
          pipe(Effect.all([rawKeyToKey(rawKey), parseVendorNumber(rawValue)])),
        ),
        Effect.allSuccesses,
        Effect.map(
          (entries) =>
  ...
  function rawKeyToKey(
    key: string,
  ): Effect.Effect<"x" | "y" | "z", ParseError> {
    switch (key) {
      case "X": {
      ...
      default: {
        return Effect.fail(
          new ParseError(`unknown key: "${key}" in "${value}"`),
        );
      }
    }
  }
