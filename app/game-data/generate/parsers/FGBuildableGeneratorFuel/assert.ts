...
export function assertVendorFGBuildableGeneratorFuel(data: unknown): asserts data is VendorFGBuildableGeneratorFuel {
  ...
  mFuel.forEach((fuel) => {
    assert(typeof fuel === "object" && fuel !== null, "mFuel must be an object");
    assert.isEmpty(
      ["mFuelClass", "mSupplementalResourceClass", "mByproduct", "mByproductAmount"].filter(
        (field) => !(field in fuel) || typeof (fuel as Record<string, unknown>)[field] !== "string",
      ),
...
export type VendorFGBuildableGeneratorFuel = VendorAbstractFGBuildableGenerator & {
  m_SFXSockets?: string;
