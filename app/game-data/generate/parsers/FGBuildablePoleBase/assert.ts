...
export function assertVendorFGBuildablePoleBase(data: unknown): asserts data is VendorFGBuildablePoleBase {
  assertVendorFGBuildable(data);

  assert.isEmpty(
    [].filter((field) => !(field in data) || typeof (data as Record<string, unknown>)[field] !== "string"),
    "Missing fields in VendorFGBuildablePoleBase",
  );
}

export type VendorFGBuildablePoleBase = VendorFGBuildable & {};
