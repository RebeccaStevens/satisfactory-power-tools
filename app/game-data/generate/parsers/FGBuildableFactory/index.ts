...
import { parseVendorBoolean, parseVendorFloat, parseVendorInt } from "~/game-data/generate/parsers/primitives";

import { assertVendorFGBuildableFactory } from "./assert";

export function parseFGBuildableFactory(data: unknown): Effect.Effect<FGBuildableFactory, ParseError> {
    ...
    Effect.all([
      ...
      pipe(
        Effect.all({
          ...
          powerConsumptionExponent: parseVendorFloat(data.mPowerConsumptionExponent),
          productionBoostPowerConsumptionExponent: parseVendorFloat(data.mProductionBoostPowerConsumptionExponent),
          ...
          canEverMonitorProductivity: parseVendorBoolean(data.mCanEverMonitorProductivity),
          canChangePotential: parseVendorBoolean(data.mCanChangePotential),
          canChangeProductionBoost: parseVendorBoolean(data.mCanChangeProductionBoost),
          ...
          productionShardSlotSize: parseVendorInt(data.mProductionShardSlotSize),
          productionShardBoostMultiplier: parseVendorFloat(data.mProductionShardBoostMultiplier),
          hasInventoryPotential: parseVendorBoolean(data.mHasInventoryPotential),
